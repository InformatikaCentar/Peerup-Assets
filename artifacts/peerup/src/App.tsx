...
const Btn = ({ label, color=C.teal, textColor=C.card, onClick=undefined, full=false, disabled=false, outline=false, small=false }) => (
          ...
          <Card key={p.id} accent={p.tip==="nudi"?C.teal:C.rose} onClick={()=>setDetaljiModal(p)}>
            <div style={{ display:"flex", gap:10 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {p.opis && <div style={{ marginTop:6, color:C.inkMid, fontSize:12, lineHeight:1.5 }}>{p.opis.slice(0,100)}{p.opis.length>100?"...":""}</div>}
          ...
          <div onClick={e=>e.stopPropagation()} style={{ background:C.card, borderRadius:"20px 20px 0 0", padding:20, width:"100%", maxWidth:430, maxHeight:"92vh", overflowY:"auto" }}>
            {uGotovo ? (
              ...
              <>
                ...
                <input ref={fileRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.docx" onChange={e=>setUFile(e.target.files?.[0]||null)} style={{ display:"none" }} />
          ...
          <div onClick={e=>e.stopPropagation()} style={{ background:C.card, borderRadius:"20px 20px 0 0", padding:24, width:"100%", maxWidth:500 }}>
            ...
            <div style={{ display:"flex", gap:14, alignItems:"center", marginBottom:14 }}>
              ...
              <div>
                ...
                <Pill color={ulogaBoja(detalji.uloga).color} bg={ulogaBoja(detalji.uloga).bg} label={ulogaBoja(detalji.uloga).label} style={{ marginTop:4 }} />
